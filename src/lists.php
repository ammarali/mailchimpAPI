...
class lists{
	...
	public function Create_list(){
		...
		$response = $this->api_Request($url, json_encode($data), 'POST');
        if(isset($response->{'id'})) {
            $this->InsertListDB($response);
        }
	}
	...
	public function Update_list() {
		...
		$response = $this->api_Request($url, json_encode($data), 'PATCH');

        if(isset($response->{'id'})) {
            $this->UpdateListDB($response, $this->list_id);
        }
    ...
    public function ShowAllLists() {
        ...
        foreach($ListData->{'lists'} as $result) {

            $list_id = $result->{'id'};
            $list_web_id = $result->{'web_id'};
            $list_name = $result->{'name'};
            $company = $result->{'contact'}->{'company'};

            array_push($this->mailchimp_lists, array('list_id' => $list_id, 'list_web_id' => $list_web_id, 'name' => $list_name, 'company_name' => $company));
    ...
    ////////////////////////// Database Operations ////////////////////////////////////////////////////////////////
    private function InsertListDB($response){

        $db = new DBClass();

        $SQL = "INSERT INTO `" . DB_PREFIX . "lists` SET list_id = '" . $db->escapeString($response->{'id'}) . "',";
        $SQL .= " list_web_id = " . (int)$response->{'web_id'} . ", name = '" . $db->escapeString($response->{'name'}) . "',";
        $SQL .= " company_name = '" . $db->escapeString($response->{'contact'}->{'company'}). "', address_1 = '" . $db->escapeString($response->{'contact'}->{'address1'}) . "',";
        $SQL .= " address_2 = '" . $db->escapeString($response->{'contact'}->{'address2'}) . "', city = '" . $db->escapeString($response->{'contact'}->{'city'}) . "',";
        $SQL .= " zip = '" . $db->escapeString($response->{'contact'}->{'zip'}) . "', state = '" . $db->escapeString($response->{'contact'}->{'state'}) . "',";
        $SQL .= " country = '" . $db->escapeString($response->{'contact'}->{'country'}) . "', phone = '" . $db->escapeString($response->{'contact'}->{'phone'}) . "',";
        $SQL .= " from_name = '" . $db->escapeString($response->{'campaign_defaults'}->{'from_name'}) . "', from_email = '" . $db->escapeString($response->{'campaign_defaults'}->{'from_email'}) . "',";
        $SQL .= " from_language = '" . $db->escapeString($response->{'campaign_defaults'}->{'language'}) . "', subject = '" . $db->escapeString($response->{'campaign_defaults'}->{'subject'}) . "',";
    ...
    private function UpdateListDB($response, $list_id){

        $db = new DBClass();

        $SQL = "UPDATE `" . DB_PREFIX . "lists` SET name = '" . $db->escapeString($response->{'name'}) . "',";
        $SQL .= " company_name = '" . $db->escapeString($response->{'contact'}->{'company'}). "', address_1 = '" . $db->escapeString($response->{'contact'}->{'address1'}) . "',";
        $SQL .= " address_2 = '" . $db->escapeString($response->{'contact'}->{'address2'}) . "', city = '" . $db->escapeString($response->{'contact'}->{'city'}) . "',";
        $SQL .= " zip = '" . $db->escapeString($response->{'contact'}->{'zip'}) . "', state = '" . $db->escapeString($response->{'contact'}->{'state'}) . "',";
        $SQL .= " country = '" . $db->escapeString($response->{'contact'}->{'country'}) . "', phone = '" . $db->escapeString($response->{'contact'}->{'phone'}) . "',";
        $SQL .= " from_name = '" . $db->escapeString($response->{'campaign_defaults'}->{'from_name'}) . "', from_email = '" . $db->escapeString($response->{'campaign_defaults'}->{'from_email'}) . "',";
        $SQL .= " from_language = '" . $db->escapeString($response->{'campaign_defaults'}->{'language'}) . "', subject = '" . $db->escapeString($response->{'campaign_defaults'}->{'subject'}) . "',";
        $SQL .= " date_updated = NOW() WHERE list_id = '" . $db->escapeString($list_id) . "';";
    ...
    public function DeleteList($list_id) {
        $db = new DBClass();
        $SQL = "DELETE FROM `" . DB_PREFIX . "lists` WHERE list_id = '" . $db->escapeString($list_id) . "';";
        $db->query($SQL);
        $db->close();
    }
    ...
    public function showList($data=array()) {
        $db = new DBClass();
        $SQL = "SELECT * FROM `" . DB_PREFIX . "lists`";
